...
const TablesManagement = () => {
  ...
  const calculateRemainingTime = useCallback(
    (session) => {
      ...
      // Don't go below 0 in display (but still allow manual close)
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSessionTimers((prev) => {
        ...
        const nowExpired = new Set(expiredSessions);
        ...
        currentSessions.forEach((session) => {
          if (session.status === "active") {
            ...
            const timeLimitMinutes = parseTimeLimit(session.time_limit);
            const isTimedSession = timeLimitMinutes > 0;
            const isNowExpired =
              isTimedSession && remaining !== null && remaining <= 0;

            if (isNowExpired && !expiredSessions.has(session.id)) {
              // First time expiring → show toast
              setToastMessage(
                `Time expired for ${session.table_name}! Please close the session.`
              );
              setShowToast(true);
              setTimeout(() => setShowToast(false), 5000);
              nowExpired.add(session.id);
            }
          }
        });

        if (nowExpired.size !== expiredSessions.size) {
          setExpiredSessions(nowExpired);
        }
  ...
  const confirmCloseAndPrint = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // 🔌 Device control
        const tableDevice = devices.find(
          (device) => device.table_id === selectedSession.table_id
        );
        if (tableDevice) {
          await toggleDevicePower(tableDevice.id, "off");
        }

        // 🧹 Clear orders for table
        await clearTableOrders(selectedSession.table_id);

        // 🧹 REMOVE SESSION FROM STATE
        setSessions((prev) => prev.filter((s) => s.id !== selectedSession.id));

        // ✅ CRITICAL: CLEAR MODAL STATES HERE
        setSelectedSession(null);
        setCustomerOrders([]);
        setShowCloseConfirm(false);

        // Optional: close table management modal too
      ...
      // No need to call setShowCloseConfirm(false) here — already done above
    }
  };
